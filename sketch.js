...
const firebaseConfig = {
  apiKey: "AIzaSyBSD00Af5hGsrzwhtFJN19UkTwnEGJOMFk",
  authDomain: "nh-inventory.firebaseapp.com",
  databaseURL:
    "https://nh-inventory-default-rtdb.asia-southeast1.firebasedatabase.app",
  projectId: "nh-inventory",
  storageBucket: "nh-inventory.firebasestorage.app",
  messagingSenderId: "446523468262",
  appId: "1:446523468262:web:0321ed991439e6ae708e27",
};
...
function saveData(data) {
  inventoryRef.set(data).catch(err => showToast('Save Error: ' + err.message));
}

// Real-time listener — all clients stay in sync
inventoryRef.on(
  "value",
  (snapshot) => {
...
function parseTranscript(text) {
  ...
  t = t.replace(
    /\b(zero|one|two|three|four|five|six|seven|eight|nine|ten|point|dot|decimal)\b/g,
    (m) => WORD_NUMBERS[m]
  );
...
function processTranscript(transcript) {
  ...
  if (!result) {
    showError(
      `Could Not Parse "${transcript}". Try: "[Bottle Name] [0.0–1.0]"`
    );
    return;
  }
  pendingParse = result;
...
function showParseConfirm({ name, level }) {
  ...
  document.getElementById("parsedLevelDisplay").textContent =
    snapped.toFixed(1);
...
document.getElementById("parsedLevel").addEventListener("input", () => {
  ...
  document.getElementById("parsedLevelDisplay").textContent = isNaN(v)
    ? "0.0"
    : v.toFixed(1);
});
...
btnConfirm.addEventListener("click", () => {
  ...
  if (!name) {
    showError("Bottle Name Cannot Be Empty.");
    return;
  }
  if (isNaN(level) || level < 0 || level > 1) {
    showError("Level Must Be Between 0 and 1.");
    return;
  }
...
function addManual() {
  ...
  if (!name) {
    showToast("Enter a Bottle Name");
    return;
  }
  if (isNaN(level) || level < 0 || level > 1) {
    showToast("Level Must Be 0–1");
    return;
  }
  applyUpdate(name, level);
  nameEl.value = "";
...
function setSort(mode) {
  ...
  document
    .getElementById("sort" + mode.charAt(0).toUpperCase() + mode.slice(1))
...
function renderInventory() {
  ...
  items.forEach((bottle) => {
    ...
    card.innerHTML = `
        ...
        <div class="bottle-name" onclick="renameBottle('${sid}')" title="Click to rename">${escHtml(
      bottle.name
    )}</div>
...
function exportXlsx() {
  ...
  if (!items.length) {
    showToast("No Bottles to Export");
    return;
  }
  ...
  const summaryRows = Object.entries(categories).map(([cat, bottles]) => ({
    ...
    "Avg Level": parseFloat(
      (bottles.reduce((s, b) => s + b.level, 0) / bottles.length).toFixed(3)
    ),
